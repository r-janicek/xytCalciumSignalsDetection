...
__all__ = ["TransformedDataset", "build_sampler",
           ...
           "arraymap", "center_crop", "random_crop"]
...
def center_crop(image, shape):
    ...
    if any(img_shp < shp for img_shp, shp in zip(image.shape, shape)):
        raise ValueError("image.shape {} < shape {}".format(image.shape, shape))
    
    start = ((image.shape[0] - shape[0]) // 2, (image.shape[1] - shape[1]) // 2)
    return image[start[0] : start[0] + shape[0], start[1] : start[1] + shape[1]]


def random_crop(image, shape):
    
    if any(img_shp < shp for img_shp, shp in zip(image.shape, shape)):
        raise ValueError("image.shape < shape")
   
    start = (np.random.randint(image.shape[0] - shape[0] + 1),
